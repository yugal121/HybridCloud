...
provisioner "remote-exec" {
    inline = [
      ...
      "sudo git clone https://github.com/yugal121/Hybrid.git /var/www/html/", 
      "sudo su << EOF",
...
resource "null_resouce" "images_repos1" {
  provision "local-exec" {
    command = "https://github.com/yugal121/Hybrid.git image1"
  }
  provision "local-exec" {
  when      = destroy
...
resource "aws_s3_bucket_object" "obj1" {
  bucket = "${aws_s3_bucket.s3-bucket1.bucket}"
  key    = "VimalSir.jpg"
...
resource "aws_cloudfront_distribution" "s3_distribution" {
  origin {
    domain_name = "${aws_s3_bucket.s3-bucket1.bucket_regional_domain_name}"
    origin_id   = aws_s3_bucket.s3-bucket1.id
  ...
  default_cache_behavior {
    ...
    target_origin_id = aws_s3_bucket.s3-bucket1.id
